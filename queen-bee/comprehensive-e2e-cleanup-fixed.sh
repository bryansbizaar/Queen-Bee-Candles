...
# COMPREHENSIVE E2E TESTING CLEANUP - FIXED PATHS
...
if [ -d "client" ]; then
    ...
    if [ -d "e2e" ]; then
        cp -r "e2e" "$BACKUP_DIR/"
        rm -rf "e2e"
        echo "✅ Removed: client/e2e/ directory"
    fi
    ...
    for file in .env.test .env.test.example; do
        if [ -f "$file" ]; then
            cp "$file" "$BACKUP_DIR/"
            rm "$file"
            echo "✅ Removed: client/$file"
        fi
    done

    # Check vite.config.js for test references
    if [ -f "vite.config.js" ]; then
        cp "vite.config.js" "$BACKUP_DIR/client-vite.config.js.backup"
        echo "📦 Backed up client/vite.config.js"
    fi
...
if [ -d "server" ]; then
    ...
    # Remove entire tests directory (THIS IS THE KEY PART!)
    if [ -d "tests" ]; then
        cp -r "tests" "$BACKUP_DIR/"
        rm -rf "tests"
        echo "✅ Removed: server/tests/ directory (including testDatabase.js and env.js)"
    fi
